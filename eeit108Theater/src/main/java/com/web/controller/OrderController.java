...
public class OrderController {
	...
	public String payByEcPay(HttpSession session, @RequestParam Integer idType, Model model) {
		...
		//可以在交易結束後觀察底下對應requestMapping的method -- receive 的system.err.println並不會出現在console中，藉此得知該方法並沒有被呼叫 -> 沒收到請求
		obj.setOrderResultURL("http://localhost:8080/eeit108Theater/order/result"); //EcPay會在付款結束後，將USER redirect至此，並附帶交易結果相關資訊
		...
		//all.aioCheckOut會根據設定好的參數產生一個html表格的字串，
		//裡面的各種input都已經設定好了，並且會自動submit，
		//只要將這個字串加為attribute並且顯示在回傳頁面上，便會自動執行。
		//EcPay End
